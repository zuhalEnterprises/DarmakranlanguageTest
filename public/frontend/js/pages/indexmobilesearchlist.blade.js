...
$.ajax({
    url: `?page=${page}${window.location.search.replace('?','&')}`,
    type: "get",
    beforeSend: function () {
...
function getMoresearchFields(et,at) {
    ...
    $.ajax({
        ...
        data:{_token:$('#js_csrf_token').val(),estateType:[0] /*(et.includes("0")?[0]:et) //in comment jahat tabdil eshterak properies be ejtema ast*/ ,activityType:at},
        error: function()
        {                
...
function searchclick(){
    ...
    if($("#hasAgent").hasClass("switchOn"))
        str+='has_agent=true&'
    str+='sortBymobile='+$("#inputGroupSelect01").val();
