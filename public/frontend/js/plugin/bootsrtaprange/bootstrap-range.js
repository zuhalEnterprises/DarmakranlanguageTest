...
(function ($) {
     ...
     * @type {{cssClass: string, minValues: Array, maxValues: Array, minPlaceholder: string, maxPlaceholder: string, minAttribute: string, maxAttribute: string, minHintText: string, maxHintText: string, readonly: boolean, format: boolean, minimumCallback: minimumCallback, maximumCallback: maximumCallback,minValue:number,maxValue:number}}
     */
    ...
    bootstrapRange.prototype.createPopover = function () {
        ...
        $target.popover({
            ...
            placement: 'bottom',
            trigger: 'manual',
            //container: '[bootstrapRange = "' + $target.attr("name")+'"]',
            content: function () {
                var minItems = '', maxItems = '';
                //prepare minimum items
                minItems += '<ul class="list-unstyled">';
                for (var i = 0; i < options.minValues.length; i++) {
                    minItems += '<li data-value="' + options.minValues[i] + '">' + (options.minValues[i] === "" ? "حذف" : options.minValues[i]) + '</li>';
                }
                minItems += '</ul>';
                ...
                for (var i = 0; i < options.maxValues.length; i++) {
                    maxItems += '<li data-value="' + options.maxValues[i] + '">' + (options.maxValues[i] === "" ? "حذف" : options.maxValues[i]) + '</li>';
                }
                maxItems += '</ul>';


                var maxHeight = ($target.offset().top - 30) + "px";

                // return html
                return '<div class="bootstrap-range" style="max-height: ' + maxHeight + ';overflow: auto;display:flex;padding:0 20px ;">' +
    ...
    bootstrapRange.prototype.initialize = function () {
        ...
        $popoverObj = classObj.popoverObj = $('.popover');
    ...
    bootstrapRange.prototype.minItemsClickHandler = function (e) {
        ...
        $minObj.val(classObj.minimumItemValue);
    ...
    bootstrapRange.prototype.maxItemsClickHandler = function (e) {
        ...
        if (!$target.hasClass('disabled')) {
            ...
            $maxObj.val(classObj.maximumItemValue);
    ...
    bootstrapRange.prototype.maxItemsHandler = function (obj) {
        var classObj = obj,
            $maxObj = classObj.maxObj;
        classObj.maximumItemValue = classObj.options.maxValue;
        //set selected maximum value to input
        $maxObj.val(classObj.format(classObj.maximumItemValue));
        classObj.setMaximum(classObj.maximumItemValue);
    ...
    bootstrapRange.prototype.minItemsHandler = function (obj) {
        var classObj = obj,
            $minObj = classObj.minObj;
        classObj.minimumItemValue = classObj.options.minValue;
        //set selected minimum value to input
        $minObj.val(classObj.format(classObj.minimumItemValue));
        classObj.setMinimum(classObj.minimumItemValue);
        classObj.checkMaxValue();
    ...
    bootstrapRange.prototype.setValue = function (e) {
        var classObj = this;
        if (classObj.options.minValue != null || classObj.options.maxValue != null) {
            classObj.initialize();
        }
        if (classObj.options.minValue != null) {
            bootstrapRange.prototype.minItemsHandler(classObj)
        }
        if (classObj.options.maxValue != null) {
            bootstrapRange.prototype.maxItemsHandler(classObj)
        }
    }
    ...
    function Plugin(options) {
        return this.each(function () {
            var tempOption = $.extend({}, bootstrapRange.DEFAULTS, options);
            var object = new bootstrapRange(this, tempOption);
            object.setCssClass();
            object.readonly();
