<!DOCTYPE html>
<html lang="{{ session('locale', app()->getLocale()) }}" dir="{{ session('locale', app()->getLocale()) == 'fa' || session('locale', app()->getLocale()) == 'ar' ? 'rtl' : 'ltr' }}">
...
<head>
    ...
    <meta name="geo.position" CONTENT="UAE">
    <meta name="geo.placename" CONTENT="Dubai">
    <meta name="geo.region" CONTENT="IR-21">
    <meta property="og:title" content="{{$title}}" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="@yield('canonical')" />
    <meta property="og:image" content="/img/site10/logo.png" />
...
<body>
    ...
    <link rel="stylesheet" media="screen" href="/css/styleRtl.css">
    @else
    <link rel="stylesheet" media="screen" href="/css/styleLtr.css">
    @endif
    ...
    <!-- Main theme script-->
    <script src="/vendor/flatpickr/dist/flatpickr.min.js"></script><script src="/vendor/flatpickr/dist/plugins/rangePlugin.js"></script><script src="/js/theme4.min.js"></script>
    ...
    <script>
        ...
        @if(app('request')->input('refid'))
            setCookie('refid', '{{app('request')->input('refid')}}', 3650);
        @endif
        ...
        $(document).ready(function () {
            ...
            $('#referrer_id').val(refid);

            // اگر کاربر لاگین نیست و refid معتبر وجود دارد، مودال را نشان بده
            @if(!$currentUser )
            //if (refid && parseInt(refid) > 0) {
                // نمایش مودال بعد از 2 دقیقه (120000 میلی‌ثانیه)
                setTimeout(function () {
            ...
            $('#leadForm').on('submit', function (e) {
                ...
                var country = $('#country_id').val().trim();  // اصلاح‌شده
                var referrer_id = $('#referrer_id').val().trim();

                if (!name || !mobile) {
                    Swal.fire({
                        icon: 'warning',
                        title: '{{l('لطفاً همه فیلدها را پر کنید')}}'
                    });
                    return;
                }
                ...
                $.post('/api/addCustomer', {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: '{{l('با موفقیت ثبت شد')}}',
                        text: '{{l('از تماس شما سپاسگزاریم')}}.'
                    });
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: l('خطا'),
                        text: '{{l('در ارسال اطلاعات مشکلی پیش آمد')}}.'
                    });
                });
            });
        });
    ...
    <script type="application/ld+json"> {
        ...
        "@type": "Organization",
        "url": "https://cafooproperties.com",
        "contactPoint": [{
          "@type": "ContactPoint",
          "telephone": "+98-25-3180",
          "email": "info@cafooproperties.com"
        }]
      } </script>
