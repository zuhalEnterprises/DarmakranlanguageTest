...
<main class="page-wrapper">
    ...
    <div class="container pt-5 pb-lg-4 mt-5 mb-sm-2">
        <!-- Page content-->
        <div class="row">
            @include('frontend.layouts.sidebar', ['menu' => 'rental_users'])
            <!-- Page content-->
            <div class="col-lg-9 col-md-12 mb-5 account add-property">
                ...
                <nav class="mb-3 pt-md-3" aria-label="Breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ l('مدیریت موجرین') }}</li>
                    </ol>
                </nav>
                    ...
                    <div class=" card-body border-0  pb-1 me-lg-1">
                        <div class="row">
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="fw-bold">{{ l('کد کاربری') }}</label>
                                <input type="text" class="form-control" id="id" name="id" />
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="fw-bold">{{ l('نام') }}</label>
                                <input type="text" class="form-control" id="name" name="name" />
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="fw-bold">{{ l('نام خانوادگی') }}</label>
                                <input type="text" class="form-control" id="last_name" name="last_name" />
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="fw-bold">{{ l('نام کاربری (موبایل)') }}</label>
                                <input type="text" class="form-control" id="username" name="username" />
                            </div>
                            
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="fw-bold">{{ l('وضعیت') }}</label>
                                <select id="status" name="status" class="form-control " style="width: 100%;" >
                                    <option value="">{{ l('انتخاب کنید') }}</option>
                                    <option value="-1" class="">{{ l('در انتظار تائید') }}</option>
                                    <option value="1" class="green">{{ l('مجاز') }}</option>
                                    <option value="2" class="orange">{{ l('معلق') }}</option>
                                </select>
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
...
<script>
    ...
    function destroy(id) {
        swal({
            text: l("آیا از حذف موجر مورد نظر اطمینان دارید؟"),
            type: 'warning',
...
<script>
    ...
    function CheckSend() {
        ...
        if ($("#name").val() != undefined && $("#name").val().length > 0)
            str += "name=" + $("#name").val() + "&&";
        if ($("#last_name").val() != undefined && $("#last_name").val().length > 0)
            str += "last_name=" + $("#last_name").val() + "&&";
