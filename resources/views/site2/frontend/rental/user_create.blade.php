...
<!-- Main Theme Styles + Bootstrap-->
    ...
    <main class="page-wrapper">
        ...
        <div class="container pt-5 pb-lg-4 mt-5 mb-sm-2">
            <!-- Page content-->
            <div class="row">
                @include('frontend.layouts.sidebar', ['menu' => 'rentalusers'])
                <!-- Page content-->
                <div class="col-lg-9 col-md-12 mb-5 account add-property">
                    ...
                    <nav class="mb-3 pt-md-3" aria-label="Breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{!empty($model)? l('ویرایش موجر') :l('ثبت موجر جدید')}}</li>
                        </ol>
                    </nav>
                    <!-- Title-->
                    <div class="mb-4">
                        <h1 class="h2 mb-0">{{!empty($model)? l('ویرایش موجر') :l('ثبت موجر جدید')}} </h1>
                    </div>
                    <!-- Basic info-->
                    @if ($errors->any())
                    ...
                    @endif
                    <form  id="js_singup-expert" role="form"  method="POST" action="<?php if (!empty($model)) echo '/rental/users/update/' . $model->id; else echo '/rental/users/store'; ?>" enctype="multipart/form-data">
                    @csrf
                    ...
                    <section class="card card-body shadow-sm p-4 mb-4" id="basic-info">
                        <h2 class="h5 mb-4"><i class="fi-info-circle text-primary fs-5 mt-n1 me-2"></i>{{l('اطلاعات موجر')}}
                        </h2>
                        <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4 mb-3">
                                ...
                                <select class="form-control user-status" name="status" id="status">
                                    ...
                                    <option value="-1" {{!empty($model) && old('status' , $model->status) == "-1" ? " selected " :''}} class="text-success">{{ l('در انتظار تائید') }}</option>
                            ...
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    @if(!empty($model) && !empty($model->photo()))
                            ...
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    <input type="file" name="profile_cover" class="dropify" id="image2" data-max-file-size="5M" data-height="300"
                                            data-default-file="{{ !empty($model) && old('profile_cover',$model->cover()) }}" value="{{ !empty($model) && old('photo',$model->cover()) }}"/>
                                    @if(!empty($model) && !empty($model->cover()))
                                    <div onclick="document.getElementById('profile_cover1').click()" data-target="#profile_cover1">
                                        <img src="{{ old('profile_cover',$model->cover()) }}" id="previewCover" style="width: 100%" />
                                    </div>
                                    @endif
                                    @if(!empty($model) && empty($model->cover()) && $model->profile_cover != null)
                                    <i class="fa-thin fa-cloud-arrow-up text-[36px]"></i>
                                    @else
                                    <a id="deleteCover" style="cursor: pointer" class="cursor-pointer absolute bottom-7 left-0 text-blue-500 text-[14px] font-light {{(!empty($model) && !empty($model->profile_cover))?'':'d-none'}}">{{ l('حذف') }}</a>
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group row">
                                    <label class="form-label fw-bold" for="temp_bio">{{ l('شماره شبا') }}</label>
                                    <input class="form-control" type="text"  id="temp_bio" name="temp_bio"  value="{{!empty($model)?$model->temp_bio:''}}">
                                </div>
                            </div>
                    ...
                    <section class="d-sm-flex justify-content-between pt-2">
                        ...
                        <button type="submit" class="btn btn-primary btn-lg d-block mb-2">
                            {{!empty($model)? l('ویرایش موجر') :l('ثبت موجر جدید')}}
                        </a>
                    </button>
                    </form>
                </div>
