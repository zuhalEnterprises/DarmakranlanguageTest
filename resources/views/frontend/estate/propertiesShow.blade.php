                        ...
                        <a class="text-body text-decoration-none" href="{{ $estate->url() }}" target="_blank">
                            ...
                            @if(ss('SITE_ID')==3 && $estate->expert_id>0 && $estate->expert && $estate->expert->isExpert() && ($estate->expiretime_expert == null || $estate->expiretime_expert > date('Y-m-d H:i:s')))
                                @if($estate->percent_expert == 50)
                                    ...
                                    @endphp
                                @endif
                                @if($estate->percent_expert == ((ss('SITE_ID') == 5 || ss('SITE_ID') == 8)?10:20))
                                    @php
                                        $classname = 'lable-silver';
                                    @endphp
                                @endif
                                @if($estate->percent_expert == 25 && (ss('SITE_ID') == 5 || ss('SITE_ID') == 8))
                                    @php
            ...
            <div class="row">
                ...
                <div class="col-6 col-md-6 col-lg-3 col-sm-6 mt-1">
                    <span class="">
                        ...
                        {{l('قیمت متری')}}:</span>
                    <h6 class="">
                        {{toPersianNumbers($estate->price_per_meter)}} {{l('تومان')}}
                    </h6>
                </div>
                @else
                @endif
            ...
            <div class="border-top pt-3 pb-2 mt-1 ">
                ...
                <div class="row">
                    <div class="col-12 col-md-6 col-lg-8 col-sm-12 mt-1">
                        ...
                        @if($currentUser->isAdmin() ||
                        $estate->percent_expert == 0 ||
                        ($estate->expert_id>0 && $estate->expert && $estate->expert->isExpert() && ($estate->expiretime_expert == null || $estate->expiretime_expert < date('Y-m-d H:i:s')) && ($currentUser->id == $estate->expert_id  ||  ((ss('SITE_ID') == 5 || ss('SITE_ID') == 8) && ($estate->percent_expert != 50 ||  $estate->created_at < date("Y-m-d", strtotime("-4 months")) ||  $estate->expiretime_expert <=  date("Y-m-d H:i:s", strtotime("+4 month", strtotime($estate->created_at)))   ) ))) ||
                        ($estate->expert_id == null || !$estate->expert || !$estate->expert->isExpert() || ($estate->expiretime_expert != null && $estate->expiretime_expert < date('Y-m-d H:i:s')))
                        )
                        {{$estate->owner_name}}
