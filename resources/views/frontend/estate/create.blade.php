    ...
    <main class="page-wrapper">
        ...
        <div class="container mt-5 mb-md-4 py-5">
            <div class="row">
                ...
                @if($currentUser->isExpert() && env('COUNTRY') != 'UAE')
                <style>
                    .necessary{background-color:#ecf4fd}
                </style>
                <div class="alert alert-info alert-dismissible fade show" role="alert">
                <span class="fw-bold">{{ l('در صورتیکه تمایل دارید به عنوان مشاور اصلی این ملک باشید, لطفا تمامی فیلدها با بکگراند رنگی را پر نمایید') }}
                ...
                <section class="card card-body shadow-sm rounded p-4 mb-4" id="basic-info">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6 col-md-3 mb-3">
                            ...
                            <input class="necessary form-control" type="text" value="{{!empty($estate) && $estate->owner_name != '09120000000'?$estate->owner_name:''}}"  id="owner_name" name="owner_name" required/>
                        </div>
                        @if($currentUser->isExpert())
                        <div class="col-sm-6 col-md-3 mb-3">
                            <label class="form-label fw-bold" for="ap-tel"> {{l('شماره تماس')}} ({{l('مالک')}})
                                @if(
                                (ss('SITE_ID') == 8 || ss('SITE_ID') == 3 || ss('SITE_ID') == 5 || ss('SITE_ID') == 2) &&
                                isset($currentUser) &&
                                ($currentUser->isAdmin() || $currentUser->isExpert()) &&
                                isset($estate) &&
                                $estate->user_id > 0 &&
                                $estate->user &&
                                $estate->user->username == $estate->phone
                            )
                            (
                                <input class="form-check-input" name="isbongah" value="1" type="checkbox" id="isbongah" {{!empty($estate) && !empty($estate->user)?($estate->user->isbongah==1?"checked":""):""}}>
                                <label class="form-check-label" for="isbongah">{{l('همکار است؟')}} </label>
                            )
                            @endif
                                <span class="text-danger">*</span>
                            </label>
                            @if(env('COUNTRY') != 'UAE')
                            <input class="necessary form-control" placeholder="09103511135" type="tel" value="{{!empty($estate) && $estate->phone != '09120000000' ? $estate->phone:''}}" required maxlength="11" minlength="11"   name="phone" id="phone" {{(!empty($estate) && ss('SITE_ID') == 3  && !$currentUser->isAdmin() && $estate->phone != '09120000000' ?"disabled":"")}} />
                            @if(!empty($estate) && $estate->phone == '09120000000' && $estate->divar != '')
                            <a target="_blank" href="https://divar.ir/v/divar/{{$estate->divar}}">
                            {{ l('آگهی دیوار') }}
                            </a>
                            @endif
                            @else
                        ...
                        @if(env('COUNTRY') != 'UAE' && ss('SITE_ID') != 8 && ss('SITE_ID') != 5 && ss('SITE_ID') != 3 && ($currentUser->isAdmin() || $currentUser->hasAnyRole('admin_super|admin_site|admin_marketing|admin_branch|expert') ))
                        <div class="col-sm-6 col-md-3 mb-3 not" access="11,16">
                            <label class="form-label fw-bold" for="ap-category">{{l('تعداد واحد تفکیکی')}}</label>
                            <input type="text" id="SeparateVilla" name="SeparateVilla"
                                   class="form-control text-left number" value="{{!empty($estate)?$estate->SeparateVilla:''}}">
                        </div>
                        @endif
                        @if(ss('SITE_ID') != 3)
                        <div class="col-sm-6 col-md-3 mb-3 not" access="11,12,13,14,15,16">
                            <label class="form-label fw-bold" for="ap-category">
                                {{l('فروش فوری')}}
                            </label>
                        ...
                        <div class="col-sm-6 col-md-3 mb-3 not" access="11,12,13,14,15,16">
                            ...
                            <select id="acquaintance_type" name="acquaintance_type" class="form-control" style="width: 100%;" required>
                                <option value="">{{ l('انتخاب کنید') }}</option>
                                @foreach(acquaintanceTypes() as $k=>$v)
                                    <option value="{{$k}}" {{!empty($estate)?($estate->acquaintance_type == $k ? "selected" : ''):''}}>{{$v}}</option>
                                @endforeach
                            </select>
                        </div>
                        @endif
                ...
                <!-- Price-->
                <section class="card card-body shadow-sm rounded p-4 mb-4" id="price">
                    ...
                    <div class="row">
                        ...
                        @if(env('COUNTRY') != 'UAE')
                        <div id="evacuationdate1" class="col-sm-6 col-md-3 col-lg-2 {{!empty($estate)?($estate->evacuation==1?'':'d-none'):'d-none'}}">
                            <label class="form-label fw-bold">
                                {{l('تاریخ تخلیه')}}
                            </label>
                        ...
                        <div class="col-sm-6 col-md-4 col-lg-4" >
                            ...
                            <select class="form-select  select3"  multiple="multiple"  name="exchange_comment[]" id="exchangetext">
                                <option value="" disabled>{{l('انتخاب تگ')}}</option>
                                @foreach($tags as $ci)
                                <option value="{{$ci->name}}" {{!empty($estate) && is_array($exchange_selected) && in_array($ci->id , $exchange_selected) ? "selected" :''}}>{{$ci->name}}</option>
                                @endforeach
                            </select>
                        ...
                        @else
                        <div class="col-sm-6 col-md-3 col-lg-2 not  {{!empty($estate)?(checkValueCreate($estate->conditions,16)=='checked'?'':'d-none'):'d-none'}}" access="11,12,13,14,15,16" id="exchangetext1">
                            <label class="form-label fw-bold">
                                {{l('توضیحات معاوضه')}}
                            </label>
                        ...
                        <div class="col-sm-6 col-md-3 col-lg-2 not" access="18">
                            <label  class="form-label fw-bold" for="delivery_date">{{l('تاریخ تحویل')}}</label>
                            <input name="delivery_date" id="delivery_date" class="form-control date-picker rounded pe-5" type="text" value="{{!empty($estate)?$estate->delivery_date:''}}" data-datepicker-options='{"altInput": true, "altFormat": "F j, Y", "dateFormat": "Y-m-d"}'>
                        </div>
                        <div class="col-sm-6 col-md-3 col-lg-2 not" access="18">
                            <label class="form-label fw-bold" for="construction_status">{{l('مرحله ساخت')}}</label>
                            <input class="form-control" value="{{!empty($estate)?$estate->construction_status:""}}" name="construction_status" />
                        </div>
                        <div class="col-sm-6 col-md-3 col-lg-2 not" access="18">
                            <label class="form-label fw-bold" for="money_paid">
                                {{l('مبلغ پرداخت اولیه')}}
                            </label>
                            <input class="form-control first-letter:number"  id="money_paid" name="money_paid"
                            placeholder="{{l('حداقل مبلغ را وارد کنید')}}" onkeypress="OnlyNumber(event,false)" onkeyup="SplitNumber($(this));"   value="{{!empty($estate)?toPersianNumbers($estate->money_paid):''}}">
                        </div>
                        @endif
                    </div>
                </section>
                <section class="card card-body shadow-sm rounded p-4 mb-4" id="location">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6 col-md-4 col-lg-2 mb-3">
                            ...
                            <select class="select2 form-select js-example-disabled-results"  name="street_id" id="street_id" aria-placeholder="test">
                                <option value="" >&nbsp;</option>
                                @if(count($streets)>0)
                                @foreach($streets as $street)
                                <option value="{{$street->id}}" {{!empty($estate) && $estate->street_id == $street->id ? "selected" :''}}>{{$street->name}}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="col-sm-6 col-md-4 col-lg-3 mb-3 not" access="11,21,16,26">
                            <label class="form-label fw-bold" >{{ l('نام مجتمع / ساختمان') }}</label>
                ...
                <section class="card card-body shadow-sm rounded p-4 mb-4" id="details">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-3 col-lg-2 mb-3 not" access="11,21,16,26">
                            ...
                            <select class="necessary form-select" id="ap-number-floor"  name="unit_in_floor" >
                                <option value="" disabled selected>{{l('انتخاب')}} </option>
                        ...
                        @if(ss('SITE_ID') == 3)
                        <div class="col-6 col-sm-3 col-md-2 not" access="11,12,13,14,15,16">
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="existing_document">{{l('سند موجود')}} </label>
                            </div>
                        </div>
                        <div class="col-6 col-sm-3 col-md-2 not" access="11,18">
                            <div class="form-check">
                                <input class="form-check-input" name="unit_in_complex" value="1" type="checkbox" id="unit_in_complex" {{!empty($estate)?(($estate->unit_in_complex == 1)?"checked":""):""}}>
                                <label class="form-check-label" for="unit_in_complex">{{l('واحد در مجتمع مسکونی')}} </label>
                            </div>
                        </div>
                        @endif
                        @if(env('COUNTRY') != 'UAE')
                            ...
                            <div class="row">
                                <div class="col-12 mb-3">
                                    <div class="row">
                                        ...
                                        @if(ss('SITE_ID') == 8 || ss('SITE_ID') == 5)
                                        <div class="col-6 col-sm-3 col-md-2 not" access="11">
                                            <div class="form-check">
                                                <input class="form-check-input" name="onebuilding" id="onebuilding" type="checkbox" value="1" {{!empty($estate)?($estate->onebuilding==1?"checked":""):""}} />
                                                <label class="form-check-label" for="onebuilding">{{ l('امکان فروش یک جا') }}</label>
                                            </div>
                                        </div>
                                        @endif
                                        <div class="col-6 col-sm-3 col-md-2  not" access="13,23">
                                            <div class="form-check">
                                                <input class="form-check-input" name="restaurant" id="restaurant" type="checkbox" value="1" {{!empty($estate)?($estate->restaurant==1?"checked":""):""}} />
                                                <label class="form-check-label" for="restaurant">{{ l('امکان رستوران') }}</label>
                                            </div>
                                        </div>
                                        @endif
                                        <div class="col-6 col-sm-3 col-md-2  not" access="11,12,13,16,21,22,23,26">
                                            <div class="form-check">
                                                <input class="form-check-input" id="facilities44" value="44" {{!empty($estate)?checkValueCreate($estate->facilities,44):""}} type="checkbox" name="facilities[]">
                                                <label class="form-check-label" for="facilities44">{{ l('کف سرامیک') }}</label>
                                            </div>
                                        </div>
                                        <div class="col-6 col-sm-3 col-md-2  not" access="11,12,13,16,21,22,23,26">
                                            <div class="form-check">
                                                <input class="form-check-input" id="facilities45" {{!empty($estate)?checkValueCreate($estate->facilities,45):""}} value="45" type="checkbox" name="facilities[]">
                                                <label class="form-check-label" for="facilities45">{{ l('کف پارکت') }}</label>
                                            </div>
                                        </div>
                                        <div class="col-6 col-sm-3 col-md-2  not" access="11,12,13,16,21,22,23,26">
                                            <div class="form-check">
                                                <input class="form-check-input" id="facilities47" {{!empty($estate)?checkValueCreate($estate->facilities,47):""}} value="47" type="checkbox" name="facilities[]">
                                                <label class="form-check-label" for="facilities47">
                                                    {{ l('کف سنگ') }}
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-6 col-sm-3 col-md-2  not" access="11,12,13,16,21,22,23,26">
                                            <div class="form-check">
                                                <input class="form-check-input" id="facilities48" value="48" {{!empty($estate)?checkValueCreate($estate->facilities,48):""}} type="checkbox" name="facilities[]">
                                                <label class="form-check-label" for="facilities48">
                                                    {{ l('کف موزاییک') }}
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-6 col-sm-3 col-md-2  not" access="11,12,13,16,21,22,23,26">
                                            <div class="form-check">
                                                <input class="form-check-input" id="facilities49" value="49" {{!empty($estate)?checkValueCreate($estate->facilities,49):""}} type="checkbox" name="facilities[]">
                                                <label class="form-check-label" for="facilities49">
                                                    {{ l('کف موکت') }}
                                                </label>
                                            </div>
                                        </div>
                                ...
                                <div class="col-sm-12 mb-3 ">
                                    ...
                                    <div class="col-6 col-sm-3 col-md-2">
                                        <div class="form-check">
                                            <input class="form-check-input"  id="condition372" value="372" type="checkbox" {{!empty($estate)?checkValueCreate($estate->conditions,372):""}} name="conditions[]">
                                            <label class="form-check-label" for="condition372">{{ l('ویژه طلاب') }}</label>
                                        </div>
                                    </div>
                                    <div class="col-6 col-sm-3 col-md-2 not" access="12">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label" for="condition383">{{ l('نیاز به بازسازی') }}</label>
                                        </div>
                                    </div>
                                    @endif
                                </div>
                                    <!--
                                    <select name="conditions[]" id="condition1" class="form-select select2 " multiple dir="rtl">
...
<!--
                                    <select name="kitchen[]" class="form-select select2 " multiple dir="rtl">
                                        ...
                                        <option value="85" {{!empty($estate)?selectValueCreate($estate->kitchen,85):""}}>{{ l('کابینت') }}</option>
                                    </select>
                                    -->
                                </div>
                    ...
                    @if(empty($estate) && ss('SITE_ID') == 5 && isset($currentUser) && ($currentUser->isAdmin() || $currentUser->isExpert()))
                    <label class="form-label fw-bold  for="ap-operation">{{ l('توضیحات مشاور') }}</label>
                    <textarea  name="operation" id="operation-state" class="js_input_max form-control " rows="5">{{old('operation')}}</textarea>
                    @endif
                </section>
                <?php if(ss('SITE_ID') != 1 && env('COUNTRY') != 'UAE'){?>
                    <section class="card card-body shadow-sm rounded p-4 mt-4" id="photos">
                    <h2 class="h5 mb-4"><i class="fi-camera-plus text-primary fs-5 mt-n1 me-2"></i>{{l('بارگزاری تصاویر')}}</h2>
                       <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            @if(ss('SITE_ID') == 8 || ss('SITE_ID')== 5)
                            @if($currentUser->isExpert())
                            <button class="nav-link" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-contact" type="button" role="tab" aria-controls="nav-contact" aria-selected="false">{{ l('تصاویر مخفی ملک') }}</button>
                            @endif
                            @endif
                                        ...
                                        if($estate!=null){
                                            ...
                                            <div id="images" class=" card mb-3">
                                                ...
                                                <div class="card-body align-content-center align-items-center d-flex flex-row flex-wrap justify-content-around">
                                                    @foreach($estate->images->where("is_360","=",0)->where("plan","=",0)->where("hidden","=",0) as $item)
                                                        <div id="media-{{$item->id}}" data-id="{{$item->id}}" class="card p-1 rounded dz-preview {{$defaultImage && $defaultImage->id == $item->id ? 'img-cover' : ''}}">
                                                            <div class="mb-0 est-container" style="cursor:pointer">
                                                                ...
                                                                <img src="{{ss('SITE_ID') == 5 ? getDomainImg($item->id) : ''}}/upload/images/estate/{{ $item->url() }}" class="w-100 est-img" style="height:250px;margin-bottom:10px">
                                                            </div>
                    ...
                    <div class=" mt-4" id="photos">
                        ...
                        if($estate!=null){
                            ...
                            if($estate->images->count() > 0){
                                ...
                                <div class="card-body align-content-center align-items-center d-flex flex-row flex-wrap justify-content-around">
                                    @foreach($estate->images->where("is_360","=",1) as $item)
