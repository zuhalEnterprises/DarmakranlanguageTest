...
'title' => l('مدیریت مشاورین و کاربران'),
])
            ...
            <div class="col-lg-9 col-md-12 mb-5 account add-property">
                ...
                <nav class="mb-3 pt-md-3" aria-label="Breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"> {{l('مدیریت مشاورین و کاربران')}}</li>
                    </ol>
                </nav>
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <h1 class="h2 mb-0">
                        {{l('مدیریت مشاورین و کاربران')}}
                    </h1>
                    <div>
                        <a href="/profile/users/create">
                    ...
                    <div class=" card-body border-0  pb-1 me-lg-1">
                        <div class="row">
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="form-label fw-bold">{{l('کد کاربری')}}</label>
                            ...
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="form-label fw-bold">{{l('نام خانوادگی')}}</label>
                                <input type="text" class="form-control" id="last_name" name="last_name" />
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="form-label fw-bold"> {{l('نام کاربری (موبایل)')}}</label>
                                <input type="text" class="form-control" id="username" name="username" />
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="form-label fw-bold">{{l('نقش کاربری')}}</label>
                                <select class="form-control" name="role_id" id="role_id" style="width:100%">

                                    <option value="9">{{l('مشاور')}}</option>
                                    @if(ss('SITE_ID') == 10 || ss('SITE_ID') == 11)
                                    <option value="10">{{l('بازاریاب')}}</option>
                                    @endif
                                    <option value="1">{{l('مدیر سایت')}}</option>
                                    <option value="0">{{l('کاربر')}}</option>
                                </select>
                            </div>
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                            ...
                            <div class="col-md-6 col-lg-3 col-sm-12 mt-3">
                                <label class="fw-bold mb-1" for="branch_id">{{ l('شعبه') }}</label>
...
<script>
    ...
    function destroy(id) {
        swal({
            text: " {{l('آیا از حذف کاربر مورد نظر اطمینان دارید؟')}}",
            type: 'warning',
    ...
    $(document).ready(function () {
        ...
        $('select.user-status').on('change', function () {
            ...
            $.get("/profile/users/status/" + user_id + "/" + user_status, function (data, status) {
                if (data.status) {
                    toast({
                        type: 'success',
                        title: data.result
...
<script>
    ...
    function CheckSend() {
        ...
        if ($("#role_id").val().length > 0)
            str += "role_id=" + $("#role_id").val() + "&&";
        if ($("#branch_id").val() != undefined && $("#branch_id").val().length > 0)
            str += "branch_id=" + $("#branch_id").val() + "&&";
