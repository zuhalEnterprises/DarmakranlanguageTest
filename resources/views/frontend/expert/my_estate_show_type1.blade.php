            ...
            <div class="border-top pt-3 pb-2 mt-1 ">
                ...
                <div class="row">
                    <div class="col-12 col-md-6 col-lg-8 col-sm-12 mt-1">
                        ...
                        @if($currentUser->isAdmin() ||
                        $estate->percent_expert == 0 ||
                        ($estate->expert_id>0 && $estate->expert && $estate->expert->isExpert() && ($estate->expiretime_expert == null || $estate->expiretime_expert < date('Y-m-d H:i:s')) && ($currentUser->id == $estate->expert_id  ||  ((ss('SITE_ID') == 8 || ss('SITE_ID') == 5) && ($estate->percent_expert != 50 ||  $estate->created_at < date("Y-m-d", strtotime("-4 months")) ||  $estate->expiretime_expert <=  date("Y-m-d H:i:s", strtotime("+4 month", strtotime($estate->created_at)))   ) ))) ||
                        ($estate->expert_id == null || !$estate->expert || !$estate->expert->isExpert() || ($estate->expiretime_expert != null && $estate->expiretime_expert < date('Y-m-d H:i:s')))
                        )
                        {{$estate->owner_name}}
