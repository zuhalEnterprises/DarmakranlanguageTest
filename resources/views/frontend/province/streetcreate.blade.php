...
'title' => (!empty($model)? l('ویرایش خیابان') :l('ثبت خیابان')),
])
            ...
            <div class="col-lg-9 col-md-12 mb-5 account add-property">
                ...
                <nav class="mb-4 pt-md-3" aria-label="Breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{!empty($model)? l('ویرایش خیابان') :l('ثبت خیابان')}}</li>
                    </ol>
                </nav>
                <!-- Page content-->
                <div class="mb-4">
                    <h1 class="h2">{{!empty($model)? l('ویرایش خیابان') :l('ثبت خیابان')}}</h1>
                </div>
                <form method="post" action="{{empty($model)?"/profile/street/store":"/profile/street/update/".$model->id}}">
                @csrf
                @method('post')
                <div class="card card-body shadow-sm rounded p-4 mb-4">
                     <div class="row">
                     <div class="col-6 col-md-6 mb-4">
                            <label for="city" class="form-label fw-bold required">{{ l('نام زیرمحله') }}</label>
                            <input id="city" type="text" class="form-control" name="name" value="{{!empty($model)?$model->name:''}}" required>
                        </div>
                        <div class="col-6 col-md-6 mb-4">
                        ...
                        <div class="col-6 col-md-6 mb-4">
                            ...
                            <select id="city_id" name="city_id" class="select2 form-control" dir="rtl" required>
                                @if(isset($cities))
                                @foreach( $cities as $item)
                                <option value="{{$item->id}}" {{$item->id == (!empty($model)?$model->city_id : '') ? 'selected' :''}}>{{$item->name}}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        <div class="col-6 col-md-6 mb-4">
                            <label for="region_number" class="form-label fw-bold required">{{ l('محله') }}</label>
                            <select id="district_id" name="district_id" class="select2 form-control" dir="rtl" required>
                                @if(isset($districts))
                                @foreach( $districts as $item)
                                <option value="{{$item->id}}" {{$item->id == (!empty($model)?$model->district_id : '') ? 'selected' :''}}>{{$item->name}}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="col-6 col-md-6 mb-4">
                            ...
                            <select class="form-control select2 adjacent_districts" name="adjacent_districts[]" id="adjacent_districts" multiple style="width:100%">
                                ...
                                <option value="{{$district->id}}"  {{!empty($model)?(($model->adjacentStreets->where('adjacent_district_id','=',$district->id)->first() != null)?"selected":''):''}}>{{$district->name}}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        <div class="col-6 col-md-6 mb-4">
                            <label class="form-label fw-bold" >{{ l('خیابان همسایه') }}</label>
                            <select class="form-control select2 adjacent_streets" name="adjacent_streets[]" id="adjacent_streets" multiple style="width:100%">
                                @if(isset($streets))
                                @foreach($streets as $street)
                                <option value="{{$street->id}}"  {{!empty($model)?(($model->adjacentStreets->where('adjacent_street_id','=',$street->id)->first() != null)?"selected":''):''}}>{{$street->name}}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div>
                            <button  type="submit" class="btn btn-primary">
                                ...
                                {{!empty($model)? l('ویرایش خیابان') :l('ثبت خیابان')}}
                            </button>
                        </div>
                     </div>
                </div>
...
function street_request(district_id, street_id){
    $.get("/api/districts/"+district_id+"/streets", function (data, status) {
        if (data.status) {
            $.each(data.result, function (i, item) {
                $('select#adjacent_streets').append($('<option>', {
                    value: i,
                    text: item
                }));
            });
            ...
            if( street_id !== '' ) {
                ...
                $("select#adjacent_streets").select2().val(street_id).trigger('change');
                $("#adjacent_streets").select2({closeOnSelect: false});
            }
        }
    });
}
...
function getStreets2(){
    $('select#district_id').on('change', function () {
        ...
        removeSelectOptions("adjacent_streets");
