...
<main class="page-wrapper">
    ...
    <div class="container pt-5 pb-lg-4 mt-5 mb-sm-2">
        <!-- Page content-->
        <div class="row">
            @include('frontend.layouts.sidebar', ['menu' => 'district'])
            <!-- Page content-->
            <div class="col-lg-9 col-md-12 mb-5 account add-property">
                ...
                <nav class="mb-4 pt-md-3" aria-label="Breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{l('لیست محلات')}}</li>
                    </ol>
                </nav>
                <div class="mb-4 d-flex align-items-center justify-content-between">
                    <h1 class="h2">{{l('محله ها')}}</h1>
                    <a href="/profile/district/create" class="btn btn-primary" >
                    ...
                    <div class=" card-body border-0  pb-1 me-lg-1">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-4 col-sm-12 mt-2">
                                <label>{{l('نام محله')}}</label>
                                <input id="name" type="text" class="form-control" name="name">
                            </div>
                            <div class="col-md-6 col-lg-4 col-sm-12 mt-2">
                                <label>{{l('نام محله (انگلیسی)')}}</label>
...
$.get("/api/provinces/1/cities" , function (data, status) {
    if (data.status) {
        $('select#city_id').append('<option value="" selected disabled>Select</option>');
...
/*$("#estate-wrapper").on('click',".del",function () {
    deleteTodoItem($(this).parent().parent().attr("id"));
    $(this).parent().parent().remove();
});*/
function deleteTodoItem(id) {
    //var id =$(item).attr('ids');
    //alert(id);
    $.get("/profile/district/destroy/"+id, function (data, status) {
        if (data.result)
        {
            swal({
                title:"{{l('محله‌ی مورد نظر با موفقیت حذف شد.')}}",
                text: "",
                type: 'success',
