...
@if($currentUser->isExpert() && ss('SITE_ID') == 3 && 0)
<div class="border rounded p-3 bg-secondary mb-2">
    <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        <div class="d-flex align-items-center gap-1">
            ...
            <p  class="m-0">اکتشافی {{(ss('SITE_ID') == 8 || ss('SITE_ID') == 5)?10:20}} درصد</p>
        </div>
    ...
    <table class="table table-bordered table-striped table-hover">
        <thead class="table-primary">
            <tr>
                ...
                @if(ss('SITE_ID') == 3)
                <th valign="middle" class="header" style="text-align:center" scope="col">
                    {{ l('بازدید کل') }}
                    <br>
                    {{ l('بازدید کارشناسان') }}
                </th>
            ...
            <tr <?php echo $estate->urgent==1 && ss('SITE_ID') == 5 ?"style='background:#a9c5f2'":""?>>
                <td valign="middle" align="center">
                    ...
                    <ul class="dropdown-menu my-1" aria-labelledby="contextMenu2">
                        ...
                        @if(
                            ...
                            <li>
                                <a class="dropdown-item" style="cursor:pointer" onclick="edit({{$estate->id}})" class="flex items-center gap-2 p-2 hover:text-blue-500">
                    ...
                    <a class="text-body text-decoration-none" href="{{ $estate->url() }}" target="_blank">
                        ...
                        @if(ss('SITE_ID')==3 && $estate->expert_id>0 && $estate->expert && $estate->expert->isExpert() && ($estate->expiretime_expert == null || $estate->expiretime_expert > date('Y-m-d H:i:s')))
                            @if($estate->percent_expert == 50)
                                ...
                                @endphp
                            @endif
                            @if($estate->percent_expert == ((ss('SITE_ID') == 8 || ss('SITE_ID') == 5)?10:20))
                                @php
                                    $classname = 'lable-silver';
                                @endphp
                            @endif
                            @if($estate->percent_expert == 25 && (ss('SITE_ID') == 8 || ss('SITE_ID') == 5))
                                @php
                ...
                <td valign="middle" align="center">
                    <a class="text-body text-decoration-none" href="{{ $estate->url() }}" target="_blank">
                        ...
                        (
                            $estate->expert_id>0 &&
                            $estate->expert && $estate->expert->isExpert() &&
                            (
                                $estate->expiretime_expert == null ||
                                $estate->expiretime_expert > date('Y-m-d H:i:s')
                            ) &&
                            (
                                $currentUser->id == $estate->expert_id  ||
                                (
                                    $currentUser->id != $estate->expert_id &&
                                    (ss('SITE_ID') == 8 || ss('SITE_ID') == 5) &&
                                    (
                                        $estate->percent_expert != 50 ||
                                        $estate->created_at < date("Y-m-d", strtotime("-4 months")) //||
                                        //$estate->expiretime_expert <=  date("Y-m-d H:i:s", strtotime("+4 month", strtotime($estate->created_at)))
                                    )
                                )
                            )
                        ) ||
                        ...
                        {{$estate->owner_name}}
                        @if($estate->isbongahi())
                        <span style="color:red">{{ l('املاکی') }}</span>
                        @endif
                        <br><br>
