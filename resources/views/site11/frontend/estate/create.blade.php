            ...
            <div class="row">
                ...
                @endif
                    <div class="row">
                        <div class="col-lg-12 add-property">
                            ...
                            <nav class="mb-3 pt-md-3" aria-label="Breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">{{!empty($estate) && app('request')->{{ l('input(\'towerid\')==0 ? l(\'ویرایش ملک\') :l(\'ثبت ملک جدید\')}}') }}</li>
                                </ol>
                            </nav>
                            <!-- Title-->
                            <div class="mb-4">
                                <h1 class="h2 mb-0">{{!empty($estate) && app('request')->{{ l('input(\'towerid\')==0? l(\'ویرایش ملک\') :l(\'ثبت ملک جدید\')}}') }}</h1>
                            </div>
                        </div>
                        @if(session('message'))
                        ...
                        <form  enctype="multipart/form-data" id="js_form_add_state" action="<?php echo empty($estate) && app('request')->input('towerid')==0 ? '/add' : '/estates/update1/' . $estate->id ?>" method="post">
                            @csrf
                            <input type="hidden" name="default_image" id="default_image">
                            <input type="hidden" id="estatelatitude" value="{{!empty($estate) && app('request')->input('towerid')==0?$estate->latitude:''}}">
                            <input type="hidden" id="estatelongitude" value="{{!empty($estate) && app('request')->input('towerid')==0?$estate->longitude:''}}">
                            <input type="hidden" name="latitude"  id="latitude" value="{{!empty($estate) && app('request')->input('towerid')==0?$estate->latitude:''}}">
                            <input type="hidden" name="longitude" id="longitude" value="{{!empty($estate) && app('request')->input('towerid')==0?$estate->longitude:''}}">
                            ...
                            <input type="hidden" name="esatateid" id="esatateid" value='{{!empty($estate) && app('request')->input('towerid')==0?$estate->id:""}}'>
                        <section class="card card-body shadow-sm rounded p-2 mb-4" id="basic-info">
                            ...
                            <!--div class="mb-3">
                                ...
                                <input class="form-control js_input_max" value="{{!empty($estate) && app('request')->input('towerid')==0?$estate->title:""}}" name="title" placeholder="{{l('عنوان آگهی')}}" />
                            </div-->
                            <div class="row">
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3">
                                    <label class="form-label fw-bold">
                                        ...
                                        <a href="/profile/manufacturer/create" target="blank" style="color: blue">[Add]</a>
                                        @endif
                                    </label>
                                    <select class="form-select select2"  name="manufacturer_id" id="manufacturer_id" name="manufacturer_id" onchange="changeManufacturer()">
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3">
                                    <label class="form-label fw-bold">
                                        {{l('پروژه')}}
                                        @if ($currentUser != null && $currentUser->isAdmin())
                                        <a href="/profile/project/create" target="blank" style="color: blue">[Add]</a>
                                        @endif
                                    </label>
                                    <select class="form-select select2"  name="project_id" id="project_id" name="project_id">
                                        <option value="">{{l('انتخاب')}}</option>
                                        @if(isset($projects))
                                        @foreach ($projects as $project)
                                        <option value="{{$project->id}}" {{!empty($estate) && $project->estate_id > 0 ? 'attr_estate="'.$project->estate_id.'"' : ''}} {{!empty($estate)?($estate->project_id==$project->id?'selected':''):''}}>{{$project->name}}</option>
                                        @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="col-sm-6 col-md-3 mb-3">
                                    ...
                                    <select class="form-control select2" name="phone2" id="phone2" style="width:100%" >
                                        <option value="">{{l('انتخاب کنید')}}</option>
                                        @foreach (country_list() as  $country)
                                        <option value="{{$country->id}}"  {{!empty($model) && $model->phone2 == $country->id ? 'selected' : ''}}>{{l($country->name)}} ({{$country->phone_code}})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-sm-6 col-md-3 mb-3">
                                ...
                                @else
                                    @if(empty($estate))
                                    <input type="hidden" name="expert_id" value="{{$currentUser != null? $currentUser->id : 0}}">
                                    @endif
                                @endif
                                @if ($currentUser != null && ($currentUser->isAdmin() || $currentUser->isExpert()))
                                @if(empty($estate))
                                ...
                                <div class="col-sm-12 col-md-12 mb-3">
                                    <div class="row">
                                        <div class="col-6 col-sm-6 col-md-3 not" access="11,12,13,14,15,16">
                        ...
                        <section class="card card-body shadow-sm rounded p-2 mb-4" id="price">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 col-md-3 col-lg-2 not" access="21,22,23,24,25">
                                    <label class="form-label fw-bold" for="ap-price">{{l('مبلغ اجاره')}}</label>
                                    <div class="d-sm-flex">
                                ...
                                <div class="col-sm-6 col-md-3 col-lg-2 not" access="21,22,23,24,25">
                                    <label class="form-label fw-bold" for="ap-price">{{l('Rent Frequency')}}</label>
                                    <div class="d-sm-flex">
                        ...
                        <section class="card card-body shadow-sm rounded p-2 mb-4" id="details">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3">
                                    ...
                                    <select class="form-select select2"  name="brand_id" id="brand_id" name="brand_id">
                                        <option value="">{{l('انتخاب')}}</option>
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3 not" access="11,12,13,15,16,21,22,23,25,26">
                                ...
                                <!--div class="col-sm-6 col-lg-3 mb-3 not" access="12,22">
                                ...
                                @if(env('COUNTRY') != 'UAE')
                                <div class="col-sm-6 col-lg-3 mb-3 not" access="11,21,22,16,26">
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3 not" access="11,21,16,26">
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3 not year-build" access="11,12,13,15,16,21,22,23,25,26">
                                ...
                                <div class="col-sm-6 col-lg-3 mb-3 not" access="21,22,23,25,26">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-12 mb-3">
                                            <div class="row">
                                                ...
                                                <div class="col-6 col-sm-6 col-md-3  not" access="11,12,13,15,16,21,22,23,25,26">
                                                    <div class="form-check">
                            ...
                            <section class="card card-body shadow-sm rounded p-2 mb-4" id="price">
                                <h2 class="h5 mb-4"><i class="fi-cash text-primary fs-5 mt-n1 me-2"></i>{{l('شبکه های اجتماعی')}}</h2>
                                <div class="row">
                                    <div class="col-sm-6 col-md-4 col-lg-3 fw-bold">
                                        <label class="form-label fw-bold">Youtube</label>
                                        <div class="d-sm-flex">
                                            <input class="form-control w-100 mb-2 " type="text" value="{{!empty($estate)?($estate->video):""}}"  id="video" name="video" >
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm-6 col-md-4 col-lg-3 fw-bold">
                                        ...
                                        <div class="d-sm-flex">
                                            <input class="form-control w-100 mb-2 " type="text" value="{{!empty($estate)?($estate->audio):""}}"  id="audio" name="audio" >
                                        </div>
                                    </div>

                                </div>
                            </section>
                            <section class="card card-body shadow-sm rounded p-2 mt-4" id="photos">
                                <h2 class="h5">
                            ...
                            <section class="d-sm-flex justify-content-between pt-2 my-4">
                                <button type="submit" class="btn btn-primary btn-lg d-block mb-2" onclick="return savecheck()" >
                                    {{!empty($estate) && app('request')->{{ l('input(\'towerid\')==0? l(\'ویرایش ملک\') :l(\'ثبت ملک\')}}') }}
                                </button>
                            </section>
...
<script>
    @if(empty($estate) && app('request')->input('towerid')==0)
        $('#project_id').on('change', function () {
            var selectedOption = $(this).find('option:selected');
            var selectedId = selectedOption.val();
            var estateAttr = selectedOption.attr('attr_estate');

            // فقط وقتی attr_estate عدد مثبت بود، ریدایرکت انجام شود
            if (selectedId && parseInt(estateAttr) > 0) {
                window.location.href = '/add?towerid=' + selectedId;
            }
        });
    @endif
        $(function () {
            $('#sortable-images').sortable({
                ...
                update: function () {
                    $('#sortable-images .image-item').each(function (index) {
                        ...
                        // مقدار hidden را به‌روزرسانی می‌کنیم: مثلا "3:1"
                        $(this).find('input[type="hidden"]').val(id + ':' + (index + 1));
                    });
                }
            });
        });
