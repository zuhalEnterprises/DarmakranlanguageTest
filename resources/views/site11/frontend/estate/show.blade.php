...
@section('main_content')
    ...
    <main class="page-wrapper">
        ...
        <section class="container">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <ul class="d-flex mb-4 list-unstyled">
                    ...
                    <li class="me-3 pe-3 border-end">
                        <b class="me-1">{{$estate->bath_count}}</b>
        ...
        @if($images->where("is_360","=",0)->count() > 0)
        <section dir="rtl" class="container overflow-auto mb-4 pb-3" data-simplebar>
            <div class="row g-2 g-md-3 gallery" data-thumbnails="true">
                ...
                @foreach ($images->where("is_360","=",0)->sortBy("priority") as $url)
                @if($count == 1)
                <div class="col-8">
                    <a class="gallery-item rounded rounded-md-3" href="{{getDomainImg($url->id)}}/upload/images/estate/{{ $url->url() }}">
                        <img class="gallery-main-img" src="" style="background-image: url('{{ crop('/upload/images/estate/'.$url->url(),950,950) }}');">
                    </a>
                </div>
                @endif
                @if($count == 2 || $count == 3)
                ...
                <div class="col-4">
                    ...
                    <a class="gallery-item rounded rounded-md-3" href="{{getDomainImg($url->id)}}/upload/images/estate/{{ $url->url() }}">
                        <img src="{{crop('/upload/images/estate/'.$url->url(),855,855)}}" class="gallery-img">
                        @php
                            $co = $images->where("is_360","=",0)->count() - 3;
                        @endphp
                        @if($co>0)
            ...
            <div class="row">
                <div class="col-md-7 mb-md-0 mb-4">
                    <div class="row">
                        <div class="col-md-6 mb-md-0 mb-4">
                            <h2 class="h4 lead">
                                @if ($estate->type == 1)
                                @if ($estate->price > 0)
                                <span class="d-inline-block ms-1 fs-lg fw-normal text-body">{{ l('قیمت:') }}</span>
                                ...
                                @endif
                                <span class="d-inline-block ms-1 fs-lg fw-normal text-body pricekey">{{l('درهم')}}</span>
                                @else
                                {{l('توافقی')}}
                                @endif
                                @endif
                    ...
                    <div class="mb-4 pb-md-3 border-top  mt-2 pt-4">
                        ...
                        <ul class="list-unstyled mt-n2 mb-0 row row-cols-2 row-cols-sm-1  row-cols-md-2">
                            <li class="mt-2 mb-0 col fs-lg "><b>{{l('نوع')}}: </b>
                                {{ estateTypes($estate->estate_type) }}
                                @if(isValueShow($estate->conditions,15))
                                <span class="badge bg-info me-2 mb-2">{{ l('پیش فروش') }}</span>
                                @endif
                            </li>
                            <li class="mt-2 mb-0 col fs-lg"><b>{{l('شماره آگهی')}}:</b> {{ $estate->id }}</li>
                            <li class="mt-2 mb-0 col fs-lg"><b>{{l('متراژ زمین')}}: </b>
                            ...
                            @if (!empty($estate->project_id) && isset($estate->project))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('پروژه')}}: </b>
                            ...
                            @if (!empty($estate->manufacturer_id))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('سازنده')}}: </b>
                            ...
                            @if (!empty($estate->brand_id))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('برند')}}: </b>
                            ...
                            @if (!empty($estate->built_area))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{ l('متراژ بنا:') }}</b> {{ $estate->built_area }}
                                </li>
                            @endif
                            @if (!empty($estate->front_area))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{ l('متراژ بر:') }}</b> {{ $estate->front_area }}
                                </li>
                            @endif
                            @if (!empty($estate->street_width))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{ l('عرض گذر:') }}</b> {{ $estate->{{ l('street_width }} متر') }}
                                </li>
                            @endif


                            @if (!empty($estate->unit_in_floor))
                                <li class="mt-2 mb-0 col fs-lg"><b> {{l('واحد در طبقه')}}: </b> {{ l($fieldList['unit_in_floor'][$estate->unit_in_floor]) }}
                                </li>
                            @endif
                            @if (!empty($estate->floor))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('شماره طبقه')}}:</b>
                                    {{ l($fieldList['floor'][$estate->floor]) }}
                                </li>
                            @endif

                            @if (!empty($estate->residence_type))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('وضعیت سکونت')}}:</b>
                                    {{ l($fieldList['residence_type'][$estate->residence_type]) }}
                                </li>
                            @endif
                            @if (!empty($estate->floor_type))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('نوع طبفات')}}:</b>
                                    {{ getvalueMeta($fieldList, $estate->floor_type, 'floor_type') }}
                                </li>
                            @endif

                            @if (false && $estate->type == 2 && !empty($estate->rent_type))
                                <li class="mt-2 mb-0 col fs-lg"><b>{{l('نوع اجاره')}}:</b>
                    ...
                    <div class="mb-4 pb-md-3">
                        ...
                        <ul class="list-unstyled row row-cols-lg-2 row-cols-md-2 row-cols-2 gy-1 mb-1 text-nowrap">
                            ...
                            <li class="col mt-3  fs-lg" style="color:#39465b">
                                ...
                                @if (!empty(IconFacility($fieldList['facilities'][$value])))
                                    <img class="mt-n1 me-2 fs-lg align-middle" src="/frontend/show/svg/{{ IconFacility($fieldList['facilities'][$value]) ?? '' }}.svg"
                                        alt="{{l($fieldList['facilities'][$value]) }}"  width="24px" height="24px" />
                                @endif
                    ...
                    <div class="card border-0 bg-secondary mb-4 rounded-3">
                        <div class="card-body">
                    ...
                    @if($estate != null && $estate->images->where("plan", "=", 1)->count() > 0)
                    ...
                    <div class="mb-4 pb-md-3">
                        ...
                        <p class="mb-1 line-h18 fulldescription fs-lg" style="text-align: justify">
                    ...
                    <div class="mb-4 pb-md-3">
                        <div class="mb-4 pb-4 border-bottom">
                            ...
                            <div class="d-flex flex-sm-row flex-column align-items-sm-center align-items-stretch justify-content-between">
                                <a class="btn btn-outline-primary mb-sm-0 mb-3" href="#modal-comment" data-bs-toggle="modal">
                ...
                <aside class="col-lg-4 col-md-5 ms-lg-auto pb-1">
                                ...
                                <div class="menu-social-container">
                                    <ul class="nav flex-row justify-content-center gap-3">
                                        <li class="nav-item fs-5">
                                            ...
                                            @else
                                                <span class="nav-link fw-normal btn-secondary btn shadow-sm ms-2 disabled" aria-label="instagram">
                                                    <i class="fi-instagram"></i> Instagram
                                                </span>
                                            @endif
                                        </li>
                                        ...
                                        <li class="nav-item fs-5">
                                            ...
                                            @else
                                                <span class="nav-link fw-normal btn-secondary btn shadow-sm ms-2 disabled" aria-label="youtube">
                                                    <i class="fi-youtube"></i> Youtube
                                                </span>
                                            @endif
                                        </li>
                    ...
                    <div class="card shadow-sm mb-4   rounded-3">
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                <a class="text-decoration-none" href="/agents/{{!empty($estate->admin)?$estate->admin->id:''}}">
                                    <div class="d-flex gap-3 justify-content-left">
                                        <img class="rounded-circle mb-2" src="{{ !empty($estate->admin) ? ($estate->admin->photo() ? $estate->admin->photo() : noImage()) : noImage() }}" width="60" alt="{{!empty($estate->admin)?$estate->admin->fullname():''}}">
                                        <div class="text-body">
                                            <h5 class="mb-1">
                                                {{!empty($estate->admin)?$estate->admin->fullname():''}}
                                            </h5>
                                            <div class="mb-1">
                                                {{ l('مشاور فروش و اجاره')}}
                                            </div>
                                        </div>
                                    </div>
                                </a>
                            ...
                            <div class="d-flex gap-1 justify-content-center mt-3">
                                @if($estate->admin->phone != '')
                                <a class="btn btn-secondary fw-bold" style="color:red" href="tel:{{!empty($estate->admin)?$estate->admin->phone:''}}">
                                    <i class="fi-phone mt-n1 me-2 align-middle opacity-60"></i>
                                    {{ l('تماس')}}
                                </a>
                                @endif

                                @if($estate->admin->whatsapp != '')
                                <a class="btn btn-secondary fw-bold" style="color:green" href="https://wa.me/{{$estate->admin->whatsapp}}">
                                    <i class="fi-whatsapp mt-n1 me-2 align-middle opacity-60" ></i>
                                    Whatsapp
                                </a>
                                @endif
                        ...
                        <div class="pt-2">
                            <div class="position-relative mb-2">
                                ...
                                <iframe class="rounded-3"
                                        ...
                                        src="https://www.google.com/maps?q={{ $estate->latitude }},{{ $estate->longitude }}&hl=fa&z=16&output=embed">
                                </iframe>

                                <!-- دکمه مسیریابی -->
                                <div class="d-flex w-100 h-100 align-items-center justify-content-center position-absolute top-0 start-0" style="top: 100px !important">
                                    <a class="btn btn-primary stretched-link"
                                    href="https://www.google.com/maps?q={{ $estate->latitude }},{{ $estate->longitude }}&hl=fa&z=16&output=embed"
                                    data-iframe="true"
                                    data-bs-toggle="lightbox">
                                    <i class="fi-route me-2"></i>{{ l('مسیریابی') }}
                                    </a>
    ...
    <div class="modal fade" id="modal-comment" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body px-sm-5 px-4">
                    <form class="needs-validation" id="js_singup-comment">
                        <div class="mb-3">
                            <label class="form-label" for="name">{{ l('نام') }} <span class="text-danger">*</span></label>
                            <input class="form-control" type="text" id="name" name="name" placeholder="{{ l('نام و نام خانوادگی') }}" required>
                            <div class="invalid-feedback">{{ l('لطفا نام خود را وارد کنید') }}</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="email">{{ l('پست الکترونیکی') }} <span class="text-danger">*</span></label>
                            <input class="form-control" type="email" id="email" name="email" placeholder="{{ l('ایمیل') }}" required>
                            <div class="invalid-feedback">{{ l('لطفا یک ایمیل معتبر وارد کنید') }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="form-label" for="body">{{ l('ثبت نظر') }} <span class="text-danger">*</span></label>
                            <textarea class="form-control" id="body" name="body" rows="5" placeholder="{{ l('توضیحات') }}" required></textarea>
                            <div class="invalid-feedback">{{ l('نظر خود را ثبت کنید') }}</div>
                        </div>
                    </form>

                    <button class="btn btn-primary d-block w-100 mb-4 btnComment" type="submit">{{ l('ثبت نظر') }}</button>
                </div>
...
@endsection
    ...
    <script>
       ...
       $(".btnComment").click(function() {
            var validate = $('#js_singup-comment').valid();

            if (validate) {
                var commentable_id = {{ $estate->id }};
                var commentable_type = 'estate';
                var body = $('#body').val();
                var name = $('#name').val();
                var email = $('#email').val();
...
<script type="text/javascript">
    ...
    height: 350,
    offset: 120,
    more: '<button class="btn btn-success btn-sm rounded px-3 shadow-sm"><i class="bi bi-chevron-down me-1"></i>{{ l('مشاهده توضیحات بیشتر') }}</button>',
    less: '<button class="btn btn-danger btn-sm rounded px-3 shadow-sm"><i class="bi bi-chevron-up me-1"></i>{{ l('مشاهده توضیحات محدود') }}</button>' }); const currency = getCookie('currency_unit'); // مثلا "dirham" const area = getCookie('area_unit'); // مثلا "feet" if (currency) { let currencyRadio = document.querySelector(`input[name="currency"][value="${currency}"]`); if (currencyRadio) currencyRadio.checked = true; } if (area) { let areaRadio = document.querySelector(`input[name="area"][value="${area}"]`); if (areaRadio) areaRadio.checked = true; } changevahed(currency , area); });
</script>
