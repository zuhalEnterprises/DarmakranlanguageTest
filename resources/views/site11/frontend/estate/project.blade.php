            ...
            <div class="col-lg-9 col-md-12 mb-5 account add-property">
                ...
                <nav class="mb-4 pt-md-3" aria-label="Breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{l('لیست پروژه ها')}}</li>
                    </ol>
                </nav>
                <!-- Page content-->
                <div class="mb-4 d-flex align-items-center justify-content-between">
                    <h1 class="h2">{{l('لیست پروژه ها')}}</h1>
                    <a href="/profile/project/create" class="btn btn-primary" >
                ...
                <div class="card shadow-sm rounded mb-4">
                    ...
                    <div class=" card-body border-0  pb-1 me-lg-1">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-lg-4 col-sm-12 mt-2">
                                <label>{{l('سازنده')}}</label>
                                <select id="manufacturer_id" name="manufacturer_id"  class="form-control select2">
                                    <option></option>
                                    @foreach ($manufacturers as $manufacturer)
                                    <option value="{{$manufacturer->id}}" {{ app('request')->input('manufacturer_id') == $manufacturer->id ? "selected":'' }}>
                                        {{$manufacturer->name}}
                                    </option>
                                    @endforeach
                                <select>
...
function CheckSend()
{
    ...
    sr += (typeof $('#manufacturer_id').val()!=='undefined' && $('#manufacturer_id').val()>0)?"manufacturer_id="+$("#manufacturer_id").val()+"&":"";
    sr+= "order="+$("#order").val()+"&";
...
function deleteTodoItem(id) {
    ...
    //alert(id);
    $.get("/profile/project/destroy/"+id, function (data, status) {
        if (data.result)
        {
            swal({
                title:"{{l('پروژه مورد نظر با موفقیت حذف شد.')}}",
                text: "",
                type: 'success',
