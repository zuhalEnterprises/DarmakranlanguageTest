...
<div class="col-sm-12 mb-3">
    <label for="header" class="form-label fw-bold">{{ l('سرگروه') }}</label>
    <select name="header" id="header" class="day-calc colorful-select dropdown-primary">
        <option value="0">{{ l('سرگروه ندارد') }}</option>
        <option value="-1" {{!empty($model) && old('header',$model->header) == -1 ? " selected " :''}}>{{ l('خود سرگروه است') }}</option>
        @if(isset($headers))
        @foreach($headers as $user)
        <option value="{{$user->id}}" {{!empty($model) && old('header',$model->header) == $user->id ? " selected " :''}}>{{$user->fullname()}}</option>
        @endforeach
        @endif
    </select>
</div>
<div class="col-sm-12 mb-3">
    <label for="header" class="form-label fw-bold">{{ l('شعبه') }}</label>
    <select name="branch_id" id="branch_id" class="day-calc colorful-select dropdown-primary">
        @foreach($branches as $branch)
        <option value="{{$branch->id}}" {{!empty($model) && old('branch_id',$model->branch_id) == $branch->id ? 'selected' :''}}>{{$branch->name}}</option>
        @endforeach
    </select>
</div>
<div class="col-sm-12 mb-3">
