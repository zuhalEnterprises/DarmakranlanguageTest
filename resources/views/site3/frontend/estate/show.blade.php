            ...
            <div class="row gy-5 pt-lg-2">
                <div class="col-lg-7">
                    <div class="d-none d-lg-flex flex-column">
                        @if (count($images->where("360","=",0)) > 0 || count($images->where("360","=",1)) > 0 || !empty($estate->video) || $estate->vrhouse)
                                ...
                                <div class="tns-carousel-wrapper">
                                    ...
                                    <div class="tns-carousel-inner"
                                        ...
                                        <a href="{{getDomainImg($url->id)}}/upload/images/estate/{{ $url->url() }}" class="fancybox.image"  data-fancybox-group="thumb" data-fancybox="gallery">
                                            <div class="rounded-3" style="background-image: url('{{getDomainImg($url->id) != 'https://file.koomeh.ir' ? crop('/upload/images/estate/'.$url->url(),850,850) : getDomainImg($url->id).'/upload/images/estate/'.$url->url() }}')"></div>
                                        </a>
                                        @endforeach
                                ...
                                <ul class="tns-thumbnails" id="thumbnails">
                                    @foreach ($images->where("360","=",0)->where("plan","=",0)->where("hidden","=",0) as $url)
                                        <li class="tns-thumbnail" >
                                                <img alt="{{ $estate->title }}" src="{{getDomainImg($url->id) != 'https://file.koomeh.ir' ? crop('/upload/images/estate/'.$url->url() , 200 , 200): getDomainImg($url->id).'/upload/images/estate/'.$url->url()}}" class="pic-estate-small">
                                        </li>
                                    @endforeach
                            ...
                            <div class="card-body ">
                                <div class="d-flex align-items-center gap-3 mb-3">
                                    ...
                                    <a class="text-decoration-none" href="/">
                                        <img class="rounded-circle mb-2 hvr-rotate" src="/img/site3/logo.jpg" width="80" height="80" alt="">
                                    </a>
                                    <div>
                                        <h5 class="mb-1">
                                        ...
                                        <a href="tel:0253180" class="text-primary text-decoration-none mb-1 fs-sm">
                                            <i class="fi-device-mobile mt-n1 me-2 align-middle opacity-70"></i>
                                            025-3180
                                        </a>
                                    </div>
                    ...
                    <div class="d-none d-lg-flex align-items-center  py-md-4 py-3">
                        <div class="d-flex flex-wrap">
                            ...
                            @if($estate->district && $estate->district->post_id != null && $estate->district->post)
                                @foreach ($estate->district->post->tags() as $tag)
                                    <a class="btn btn-sm btn-outline-secondary rounded-pill fs-xs fw-normal me-2 mb-2"
                                    href="{{$estate->district->post->url()}}" target="_blank">
                                        {{$tag->name}}
                                    </a>
                                @endforeach
                            @endif
                            @if($estate->street && $estate->street->post_id != null && $estate->street->post)
                                @foreach ($estate->street->post->tags() as $tag)
                                    <a class="btn btn-sm btn-outline-secondary rounded-pill fs-xs fw-normal me-2 mb-2"
                                    href="{{$estate->street->post->url()}}" target="_blank">
                                        {{$tag->name}}
                                    </a>
                                @endforeach
                            @endif
                    ...
                    <div>
                        <div class="position-relative">
                            ...
                            <p class="mb-4 fs-5 fw-bold">
                                ...
                                @else
                                آدرس:
                                @if($estate->city && $estate->city->name)
                                    @if(!$estate->city->post_id || !$estate->city->post)
                                        {{ $estate->city->name }}
                                    @else
                                ...
                                @if($estate->district && $estate->district->name)
                                    @if(!$estate->district->post_id || !$estate->district->post)
                                ...
                                @if($estate->street && $estate->street->name)
                                    @if(!$estate->street->post_id || !$estate->street->post)
                                        {{ " - " . $estate->street->name }}
                                    @else
                        ...
                        @if (count($images->where("360","=",0)) > 0 || 1)
                            <!-- Carousel with slides count-->
                            <div class="order-lg-1 order-2">
                                <div class="tns-carousel-wrapper">
                                    <div class="tns-slides-count text-light"><i class="fi-image fs-lg me-2"></i>
                ...
                <div class="tns-carousel-wrapper tns-controls-outside-xxl tns-nav-outside tns-nav-outside-flush mx-n2">
                    <div class="tns-carousel-inner row gx-4 mx-0 pt-3 pb-4"
                        ...
                        @foreach ($similarEstates as $estat)
                            <div class="col"  data-aos-once="true" data-aos="zoom-in" data-aos-duration="1000"  data-aos-delay="300">
