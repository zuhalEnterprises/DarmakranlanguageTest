...
<div class="card shadow-sm card-hover border-0 h-100 hvr-grow">
                ...
                <h3 class="h6 mb-2 fs-base">
                    <a target="_blank" class=" text-white" href="{{$estate->url()}}">
                        ...
                        @if($estate->city->name)
                            ...
                            @else
                            <a class="text-white"
                            href="{{ $estate->city && $estate->city->post ? $estate->city->post->url() : '#' }}"
                            target="_blank"
                            title="املاک {{ estateTypes($estate->estate_type) }} {{ $estate->city->name ?? '' }}">
                                {{ $estate->city->name}}
                            </a>
                            @endif
                        @endif
                        @if($estate->district && $estate->district->name)
                            ...
                            <a class="text-white"
                            href="{{ $estate->district && $estate->district->post ? $estate->district->post->url() : '#' }}"
                            target="_blank"
                            title="املاک {{ estateTypes($estate->estate_type) }} {{ $estate->district->name ?? '' }}">
                        ...
                        @if($estate->street && $estate->street->name)
                            ...
                            <span class="text-white">-</span>
                            <a class="text-white"
                            href="{{ $estate->street && $estate->street->post ? $estate->street->post->url() : '#' }}"
                            target="_blank"
                            title="املاک {{ estateTypes($estate->estate_type) }} {{ $estate->street->name ?? l('نامشخص') }}">
                            {{ $estate->street->name ?? '' }}
                            </a>
                            @endif
                        @endif
