...
@section('main_content')
    ...
    <main class="page-wrapper">
                ...
                <div class="col-lg-7">
                    <div class="d-flex flex-column">
                            ...
                            <div class="order-lg-1 order-2">
                                <div class="tns-carousel-wrapper">
                                    ...
                                    <div class="tns-carousel-inner"
                                        ...
                                        @endforeach
                                        @if(ss('SITE_ID') == 5 && $currentUser && ($currentUser->isExpert() || $currentUser->isAdmin()))
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-3 d-block d-lg-none">
                            <div>
                                <span class="badge bg-success me-2 mb-2">{{ estateTypes($estate->estate_type) }}</span>
                                <span class="badge bg-info me-2 mb-2 confirmationspan">{{confirmStatuses($estate->confirmation)}}</span>
                                @if(ss('SITE_ID') == 5 && $estate->urgent == 1)
                                <span class="badge bg-warning me-2 mb-2">{{ l('ملک ویژه(فوری)') }}</span>
                                @endif
                            </div>
                            <div class="d-flex flex-wrap text-nowrap">
                                @if($edit)
                                <a class="btn btn-light-primary btn-xs shadow-sm ms-2 mb-2 bg-warning" href="/profile/my-estate-ads"
                                    type="button" data-bs-toggle="tooltip"  aria-label="{{ l('ویرایش و حذف ملک') }}" title="{{l('ویرایش و حذف ملک')}}">
                                    {{ l('ویرایش و حذف ملک') }} <i class="fi-edit"></i>
                                </a>
                                @endif
                                ...
                                <button
                                    class="btn btn-icon btn-light-primary btn-xs shadow-sm rounded-circle ms-2 mb-2 archive"
                    ...
                    <!-- Post meta-->
                    <!-- Reviews-->
                    @if((ss('SITE_ID') == 3 || ss('SITE_ID') == 5) && $currentUser && ($currentUser->isExpert() || $currentUser->isAdmin()))
                    <div class="card border-0 bg-secondary mb-4">
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h5 class="mb-0">{{ l('عملکرد') }}</h5>
                    ...
                    @if(ss('SITE_ID') == 5 && $currentUser && ($currentUser->isExpert() || $currentUser->isAdmin()))
                    <div class="mb-4 pb-4 border-bottom d-none" >
                        <div class="d-flex flex-sm-row flex-column align-items-sm-center align-items-stretch justify-content-between">
                            <a class="btn btn-outline-primary mb-sm-0 mb-3" href="#modal-relation" data-bs-toggle="modal">
                                <i class="fi-edit ms-1"></i>{{ l('انتقال به املاک مناسب') }}
                            </a>
                ...
                <aside class="col-lg-5">
                    <div class="ps-lg-5">
                        ...
                        <div class="card border-0 bg-secondary mb-4">
                            <div class="card-body">
                                ...
                                <ul class="list-unstyled mt-n2 mb-0 row  row-cols-2">
                                    ...
                                    <li class="mt-2 mb-0 col"><b>{{ l('متراژ:') }}</b> {{ $estate->area }} {{l('مترمربع')}}</li>
                                    ...
                                    @if (!empty($estate->unit_in_floor))
                                        <li class="mt-2 mb-0 col"><b>{{ l('تعداد واحد در طبقه:') }}</b> {{ $fieldList['unit_in_floor'][$estate->unit_in_floor] }}
                                        </li>
                                    @endif
                        ...
                        <div class="card border-0 bg-secondary mb-4">
                            <div class="card-body">
                                ...
                                <div class="m-0">
                                    ...
                                    <p class="mb-1"> <span class="fw-bold">{{ l('تاریخ آخرین ویرایش') }}</span>: {{!empty($dateend)?toPersianDate($dateend):"-"}} </p>
                                    <p class="mb-1"> <span class="fw-bold">{{ l('آخرین ویرایش کننده') }}</span> : {{!empty($estateuser)?$estateuser->fullname():"-"}} </p>
                                </div>
                            </div>
                        </div>
                        @endif
                                ...
                                <div class="card-body">
                                    ...
                                    <ul class="list-unstyled row row-cols-md-2 row-cols-1 gy-2 mb-0 text-nowrap">
                                        @foreach (json_decode($estate->conditions, true) as $value)
                                            ...
                                            <li class="col">
                                                @if (!empty($fieldList['conditions'][$value]))
                                                   {{$fieldList['conditions'][$value]}}
                                                @endif
                                            </li>
                                        @endforeach
                                    </ul>
                        ...
                        @foreach ($images->where("plan","=",1) as $url)
                            <img src="{{getDomainImg($url->id)}}/upload/images/estate/{{ $url->url() }}" style="height: 200px;width:100%;object-fit:cover" data-src="" alt="">
                        @endforeach
                            </div>
                        </div>
                        @endif
                ...
                <div class="card-body ">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            ...
                            <a class="text-decoration-none" href="/">
                                <img class="rounded-circle mb-2" src="/img/site3/logo-sha.webp" width="80" height="80" alt="">
                            </a>
                            <div>
                                <h5 class="mb-1">
                                ...
                                <a href="tel:02532611515" class="text-primary text-decoration-none mb-1 fs-sm">
                                    <i class="fi-device-mobile mt-n1 me-2 align-middle opacity-70"></i>
                                    025-32611515
                                </a>
                            </div>
        ...
        @if(count($relationCustomers)>0 && ss('SITE_ID') != 4)
                            ...
                            <div class="col">
                                <div class="card shadow-sm card-hover border-0 h-100">
                                    <div class="card-img-top card-img-hover">
                                        <a class="img-overlay" href="/v/{{ $estat->id }}" aria-label="@if (!empty($estat->title))
                                            {{ $estat->title }}
                                        @endif
                                    ...
                                    <div class="card-body position-relative pb-3">
                                        <h3 class="h6 mb-2 fs-base">
                                            <a class="nav-link stretched-link" href="/v/{{ $estat->id }}">
                    ...
                    <div class="card  bg-white  p-4">
                        ...
                        <div class="tns-carousel-inner row gx-4 mx-0 pt-3 pb-4"
                                ...
                                <div class="col">
                                    <div class="card shadow-sm card-hover border-0 h-100">
                                        <div class="card-img-top card-img-hover">
                                            <a class="img-overlay"  href="/v/{{$estate1->id}}/{{$estate1->link_rewrite()}}"></a>
                                            <div class="position-absolute start-0 top-0 pt-3 pe-3">
                                                <span class="d-table badge bg-info mb-1">
                                                    {{toPersianDate($estate1->showdate)}}
                                                </span>
                                                <span class="d-table badge bg-primary">{{estateTypes($estate1->estate_type)}}</span>
                                            </div>
                                            <div class="content-overlay end-0 top-0 pt-3 ps-3">

                                            </div>
                                            <img src="{{$estate1->coverImage()}}" style="height: 200px;width:100%;object-fit:cover"
                                                data-src="{{$estate1->coverImage()}}" alt="{{!empty($estate1->title) ? $estate1->title : estateTypes($estate1->estate_type) . l('در') . ($estate1->district->name ?? '') . ' ' . ($estate1->city->name ?? '')}}">
                                        </div>
                                        <div class="card-body position-relative pb-3">
                                            <h3 class="h6 mb-2 fs-base">
                                                <a class="nav-link stretched-link" href="/v/{{$estate1->id}}/{{$estate1->link_rewrite()}}">
                                                    {{!empty($estate1->title) ? $estate1->title : estateTypes($estate1->estate_type) . l('در') . ($estate1->district->name ?? '') . ' ' . ($estate1->city->name ?? '')}}</a>
                                            </h3>
                                            <div class="d-flex justify-content-between align-content-center">
                                                <div><i class="fi-cash lead align-middle opacity-70"></i>
                                                    @if ($estate1->price > 0)
                                                    {{ toPersianNumbers($estate1->price) }} {{l('تومان')}}
                                                    @else
                                                    {{l('توافقی')}}
                                                    @endif
                                                </div>
                                                <div><i class="fi-cash lead align-middle opacity-70"></i> متری:
                                                    {{ $estate1->price_per_meter == 0 ? '' : toPersianNumbers($estate1->price_per_meter) }} {{ l('تومان') }}
                                        ...
                                        <div class="card-footer d-flex align-items-center justify-content-between pt-3 text-nowrap">
                                            <div>
                                                ...
                                                @if(!empty($estate1->geography))
                                                <span class="d-inline-block px-2 fs-sm">
                                                    <i class="fi-geo me-1 mt-n1 fs-lg text-muted"></i>
                                                    {{ $fieldList['geography'][$estate1->geography] }}
                                                </span>
                                                @endif
                                                <span class="d-inline-block px-2 fs-sm">
                                                    <i class="fi-clock me-1 mt-n1 fs-lg text-muted"></i> ساخت:
                                                    {{ buildYear($estate1->built_year) }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
...
<div class="modal fade" id="modealmoarefi" tabindex="-1" role="dialog" aria-labelledby="exampleModalPreviewLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary moarefisend">{{ l('بله') }}</button>
            </div>
            </div>
    </div>
</div>

<div class="modal fade" id="modalarchive" tabindex="-1" role="dialog" aria-labelledby="exampleModalPreviewLabel"
     aria-hidden="true" style="z-index: 99999;top:0%;bottom:0% !important;height:auto">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <h3>{{ l('آیا از آرشیو کردن مطمئن هستید؟') }}</h3>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ l('بستن') }}</button>
                <button type="button" class="btn btn-primary archiveend">{{ l('بله') }}</button>
            </div>
            </div>
    </div>
</div>

<div class="modal fade" id="modalrelationsms" tabindex="-1" role="dialog" aria-labelledby="exampleModalPreviewLabel"
aria-hidden="true" style="z-index: 99999;top:0%;bottom:0% !important;height:auto">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ l('ارسال پیامک') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body px-sm-5 px-4">

                    <div class="mb-3 ">
                    <label class="form-label" for="customer_id5">{{ l('نام مشتری') }}</label>
                        <select class="form-control" id="customer_id5" name="customer_id5">
                            @if($customers)
            ...
            <div class="modal-footer">
                <button class="btn btn-primary d-block w-100 mb-4 smsrelationend" type="submit">{{ l('ارسال پیامک') }}</button>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="modal-body px-sm-5 px-4">
                <div class="mb-3">
                    ...
                    <select class="form-control form-select" id="type" name="type">
                        <option value="1" >{{ l('کارشناسی تخصصی') }}</option>
                        <option value="4" >{{ l('سایر توضیحات') }}</option>
                        <option value="2" >{{ l('بازدید') }}</option>
                        <option value="7" >{{ l('قرار بازدید') }}</option>
                    </select>
                </div>
                <div class="mb-3 " id="customer_id" style="display:none">
                    <label class="form-label" for="customer_id">{{ l('نام مشتری') }}</label>
                    <select class="form-control  form-select w-100 select2 customer_id" id="customer_id"  style="width: 100%!important;height:40px!important" name="customer_id">
                        <option value="" id="lblCustomer">{{ l('بدون همراهی با مشتری') }}</option>
                        @if($customers)
                        @foreach ($customers as $customer)
                        <option value="{{$customer->id}}" >{{$customer->name}}</option>
                        @endforeach
                ...
                <div class="mb-4">
                    <label class="form-label" for="comment" id="lblComment">{{ l('توضیحات') }}</label>
...
@if(ss('SITE_ID') == 5 && $currentUser && ($currentUser->isExpert() || $currentUser->isAdmin()))
<div class="modal fade" id="modal-relation" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header d-block position-relative border-0 pb-0 px-sm-5 px-4">
                <h4 class="modal-title mt-4 text-center font-vazir">{{ l('انتقال به املاک مناسب') }}</h4>
                <button class="btn-close position-absolute top-0 end-0 mt-3 me-3" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body px-sm-5 px-4">
                <div class="mb-3 " >
                    <label class="form-label" for="customer_id2">{{ l('نام مشتری') }}</label>
                    <select class="form-control form-select customer_id2" id="customer_id2" name="customer_id2">
                        @if($customers)
                        @foreach ($customers as $customer)
                        <option value="{{$customer->id}}" >{{$customer->name}}</option>
                        @endforeach
                ...
                <button class="btn btn-primary d-block w-100 mb-4 btnRelation" type="submit">{{ l('ثبت') }}</button>
            </div>
        </div>
    </div>
</div>
...
@section('js')
    ...
    <script>
            ...
            $("#type").on( "change", function() {
                ...
                switch (currentValue) {
                    ...
                    case '7':
                        $('#customer_id').show();
                        $('#lblCustomer').hide();
                        $('#lblComment').html('ساعت زمان حضور');
                        break;
                    default:
                    $('#customer_id').hide();
                    $('#lblCustomer').show();
        ...
        $(".smsrelationend").click(function()
        {
            estate_id = {{ $estate->id }};
            customer_id = $('#customer_id5').val();
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': '{{csrf_token()}}'
                }
            ...
            $.ajax({
                url: '/sendRelationEstate/'+customer_id+'/'+estate_id,
                type: "GET",
                success: function(data) {
                    $('#customer_id5').val('');
                    $('#modalrelationsms').modal('toggle');
                    swal({
                        title: l("پیامک با موفقیت اضافه گردید"),
                        message: "",
                        confirmButtonColor: '#025EC6',
                        confirmButtonText: l('باشه'),
                        type: "success",
        ...
        $.ajaxSetup({
            ...
            $.ajax({
                ...
                success: function(data) {
                    //var res = data.data;
                    //var operation = res.operation_id;
                    $('.confirmationspan').html('آرشیو شده');

                    swal({
                        title: "{{l('ملک با موفقیت آرشیو گردید')}}",
                        message: "",
                        confirmButtonColor: '#025EC6',
                        confirmButtonText: l('باشه'),
                        type: "success",
    ...
    // print "false" if direction is down and "true" if up
        if(this.oldScroll < this.scrollY){
            ...
            {
                setv = false;
                $.get("/estates/setVisit/{{$estate->id}}", function(data, status) {

                });
                /*$.ajaxSetup({headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}});
                $.ajax({
                    url: '/estate/setVisit/{{$estate->id}}',
                    type: "GET",
                    success: function(data) {
...
<script type="application/ld+json">
    [
        ...
        "url":"/v/{{$estate->id}}/{{$estate->link_rewrite()}}",
        ...
        "description":"{{$estate->description}}"
            ...
            "itemListElement":[
                ...
                },
                {
                    "@type":"ListItem",
                    "item":{
                        "@id":"/v/{{$estate->id}}/{{$estate->link_rewrite()}}",
                        "name":"{{!empty($estate->title) ? $estate->title : estateTypes($estate->estate_type) . l('در') . ($estate->district->name ?? '') . ' ' . ($estate->city->name ?? '')}}",
                        "@type":"Thing"
                    },
                    "position":6
