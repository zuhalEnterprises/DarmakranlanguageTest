...
<header class="navbar navbar-expand-lg navbar-light bg-light fixed-top border-bottom flex-column py-0" data-scroll-header>
    ...
    <div class="container py-2">
        ...
        <a class="btn btn-sm ms-auto" data-bs-toggle="modal" data-bs-target="#provinceModal" >
       ...
       <!--  <a class="btn btn-primary btn-sm ms-2 order-lg-3 d-none d-lg-block" href="/add"><i class="fi-plus me-2"></i>{{ l('ثبت رایگان ملک') }}</a>
        ...
        <a class="p-3  order-3 rounded-4 align-items-center justify-content-center me-2 d-none d-lg-flex" style="width: 40px; height:40px;background:#dadada;" href="">
            <i class="fi-chat-left text-dark"></i>
        </a>
                            <div class="dropdown">
        ...
        <div class="collapse navbar-collapse order-lg-2" id="navbarNav2">
            <ul class="navbar-nav navbar-nav-scroll m-0" style="max-height: 35rem;">
                ...
                <li class="nav-item dropdown d-lg-none px-0">
                    <a class="dropdown-item d-flex align-items-center gap-2" href="/profile/branch-estate-ads">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <ul id="province-list" class="list-group">
                        ...
                        <li class="list-group-item province">{{$province->name}}</li>
                        @endforeach
    ...
    <script>
        $(document).ready(function () {
            ...
            var cities = {
                ...
                @endforeach
                //"تهران": {'tehran':l("تهران"), 'parand':l("پرند"), 'shariyar':l("شهریار")},
                //"قم": ["قم", l("کهک"), l("جعفریه")],
                //"اصفهان": ["کاشان", l("شهررضا"), l("اصفهان")]
                // اینجا می‌توانید شهرهای استان‌های دیگر را اضافه کنید
            };
            // متغیر برای ذخیره شهرهای انتخاب شده
            var selectedCities = [{{$selectedCity}}];
            var temporarySelectedCities = [];
            // وقتی روی یک استان کلیک می‌کنید
            $('#province-list').on('click', '.province', function () {
                var province = $(this).text().trim();
                var citiesOfProvince = cities[province];

                $('#modalTitle').text(province);
                $('#province-list').hide();
                $('#city-list').empty();
                // شهرهای مربوط به استان انتخاب شده را نمایش می‌دهیم
                $.each(citiesOfProvince, function (index, city) {
                    var checked = selectedCities.includes(city) ? 'checked' : '';
                    $('#city-list').append('<div class="form-check"><input class="form-check-input city-checkbox" type="checkbox" value=' + index + ' id=' + index + ' ' + checked + '><label class="form-check-label" for="' + checked + '">' + city + '</label></div>');
                });
                $('#city-list').show();
            ...
            function updateSelectCityButton() {
                ...
                if (selectedCities.length > 0) { if (selectedCities.length === 1) { $('#selectCityButton').text(selectedCities); } else { $('#selectCityButton').text(selectedCities.length + ' شهر'); } } else { $('#selectCityButton').text('انتخاب شهر'); } } // تابع برای به روزرسانی لیست شهرهای انتخاب شده در مدال function updateSelectedCitiesList() { console.log(selectedCities); $('#selected-cities-list').empty(); $.each(selectedCities, function (index, city) { $('#selected-cities-list').append('<li class="d-flex align-items-center gap-1 border rounded-4 px-2 border-danger">' + allcities[city] + ' <div class="text-danger delete-city" data-city="' + city + '"><span class="fi-x"></span></div></li>');
                });
            }
            ...
            $('#updateSelectCityButton').on('click', function () {
                ...
                setCookie('city',selectedCities,365);
                window.location = '/cs/'+selectedCities;
            });
            ...
            $(document).on('click', '.delete-city', function () {
                ...
                console.log(selectedCities);
                selectedCities.splice(cityToRemove, 1);
                var index = selectedCities.indexOf(cityToRemove.toString());
                console.log(index);
                if (index !== -1) {
                    selectedCities.splice(index, 1);
                }
            ...
            $('#provinceModal').on('shown.bs.modal', function () {
                ...
                if (selectedCities.length > 0) {
                    ...
                    $.each(selectedCities, function (index, city) {
                        $('#selected-cities-list').append('<li class="d-flex align-items-center gap-1 border rounded-4 px-2 border-danger">' + allcities[city] + ' <div class="text-danger delete-city" data-city="' + city + '"><span class="fi-x"></span></div></li>'); }); } }); // وقتی مدال بسته شده است $('#provinceModal').on('hidden.bs.modal', function () { $('#city-list').empty(); $('#province-list').show(); $('#backButton').hide(); $('#selectCityButton').show(); $('#modalTitle').text('لیست استان‌ها'); $('#searchInput').val(''); }); // وقتی دکمه برگشت کلیک می‌شود $('#backButton').on('click', function () { $('#city-list').hide(); $('#backButton').hide(); $('#selectCityButton').show(); $('#modalTitle').text('لیست استان‌ها'); $('#province-list').show(); }); });
</script>
<script>
    var str = "";
    $(document).ready(function(){
